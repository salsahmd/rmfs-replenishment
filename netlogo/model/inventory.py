...
class Inventory(Universe):
    ...
    def __init__(self):
        super().__init__()  # Universe.__init__ sets self._objects = []
        self._tick = 0
    ...
    def finish_picking_task(self, job: RobotJob):
        ...
        for order_id, sku, quantity in job.orders:
            ...
            if order is None:
                print(f"[WARN] finish_picking_task: order {order_id} not found, skipping sku {sku}")
                continue
            if not order.has_sku(sku):
                print(f"[WARN] finish_picking_task: order {order_id} does not have sku {sku}, skipping")
                continue
