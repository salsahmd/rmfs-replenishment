...
POD_SIZE_PATH   = PROJECT_ROOT / "pods_dictionary.csv"   # uses pods_dictionary format
ITEMS_DICT_PATH = PROJECT_ROOT / "sku_sample.csv"

OUT_ALL_CONFIG  = BASE / "items_slots_configuration.csv"
OUT_BEST_CONFIG = BASE / "best_config_per_sku.csv"

# pod_type to use from pods_dictionary.csv
POD_TYPE = 0
...
def load_items(path: Path) -> pd.DataFrame:
    """
    Loads sku_sample.csv and applies robust dimension parsing.
    ...
    if missing:
        raise ValueError(
            f"items file missing columns: {missing}. Found: {list(df.columns)}"
        )
...
def main():
    ...
    slot_df  = load_pod_size(POD_SIZE_PATH, pod_type=POD_TYPE)
